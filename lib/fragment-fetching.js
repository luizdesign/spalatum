...
const ContentTypeError = require('./exceptions/contentTypeException');
...
const fragmentFetching = async (fragmentAttributes) => {
  ...
  if (fragmentAttributes.primary && response.status !== 200) {
    const errorMessage = `Spalatum cant render the primary fragment (${fragmentAttributes.href}), the returned statusCode was ${response.status}.`;
