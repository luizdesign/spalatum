...
const FactoryFetchOptions = (fragmentAttributes) => {
  const fetchOptions = new URL(fragmentAttributes.uri);

  if (fragmentAttributes.proxy) {
    fetchOptions.path = fragmentAttributes.uri;
...
const fetchFragment = (fragmentAttributes) => {
  ...
  const requestFn = requestFns[fetchOptions.protocol];

  return new Promise((resolve, reject) => {
    requestFn.get(fetchOptions, (res) => {
